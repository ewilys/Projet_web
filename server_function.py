...
def getMemberProfile(login): 
	...
	try: 
		print(login)
		row= db.execute('SELECT m.nom, m.prenom, ca.categorie, c.nom_club, m.email, m.date_n FROM Membres AS m, Categories AS ca, Clubs AS c, Connex_Membre AS mc WHERE m.licence=ca.licence AND m.club_id=c.club_id AND m.licence=mc.licence AND mc.login_membre=:who',{"who":login}).fetchone()
		if row is not None:
			print(row) #debug
			return row
		else: 
